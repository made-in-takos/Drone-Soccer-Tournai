/*
  Manette 2 — Goal BLEU — Drone Soccer (Pro Mini LGT8F328P)

  Carte : LGT8F328P (coeur lgt8fx)

  Envoie au GOAL ROUGE (maitre) via 433 MHz :
    B+   but equipe bleue
    B-   correction but bleu
    RST  remise a zero (les deux scores)

  Le goal rouge met a jour l'app Bluetooth, envoie SYNC et LED:BLU
  au goal bleu (esclave) qui allume ses propres LEDs.

  === BRANCHEMENTS (identique manette rouge) ===
  ...
  RF_BITRATE 2000 — identique goal rouge et manette rouge.
*/
...
#define RF_SEND_REPEAT   3
#define RF_SEND_GAP_MS   30
...
void rfSend(const char *msg) {
  ...
  for (int i = 0; i < RF_SEND_REPEAT; i++) {
    rf.send((uint8_t *)msg, len);
    rf.waitPacketSent();
    delay(RF_SEND_GAP_MS);
  }
}
...
void setup() {
  ...
  if (!rf.init()) {
    ...
    Serial.println("Manette BLEU prete (B+ B- RST -> goal rouge)");
  }
}
...
void loop() {
  ...
  if (boutonOk(BTN_PLUS, now)) {
    envoyer(BTN_PLUS, "B+", ledBleu);
    return;
  }
  if (boutonOk(BTN_MINUS, now)) {
    envoyer(BTN_MINUS, "B-", ledRouge);
    return;
  }
  if (boutonOk(BTN_RESET, now)) {
    envoyer(BTN_RESET, "RST", ledVert);
  }
}
